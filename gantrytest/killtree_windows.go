...
// childGroup ties an app under test to a Windows Job Object with
// kill-on-close: the app spawns helper windows (--shellrole children)
// and WebView2 processes, so a plain Process.Kill would orphan them.
// Every process assigned to the job - and all its descendants - dies
// when the job handle closes, including when the test binary crashes.
// Same machinery as the CLI's dev child management.
