// The React half of the index page. usePaired() connects to the .go
// file next door: send() fires its handlers, state mirrors what Go
// pushes with the "state" event.
import { useState } from "react";
import { usePaired, ExternalLink } from "gantry-web";
      ...
      <div className="card">
        <button
          onClick={() => {
            const n = count + 1;
            setCount(n);
            send("buttonPress", n); // lands in index.go - check the log
          }}
        >
          count is {count}
        </button>
        <p>
          Edit <code>pages/index/index.tsx</code> and save to test HMR
        </p>
      </div>
