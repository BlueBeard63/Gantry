...
// at commit 6173450 (see LICENSE), patched to link webkit2gtk-4.1 by
// default: upstream hardcodes the webkit2gtk-4.0 pkg-config name, which
// modern distros (Fedora 40+, Ubuntu 24.04+) no longer ship. The
// pkg-config flags live in cgo_webkit41_linux.go / cgo_webkit40_linux.go
// so the webkit2gtk40 build tag can select the legacy name; everything
// else is upstream verbatim, minus the darwin/windows paths (Windows
// uses go-webview2, macOS has no native shell).
package webview
