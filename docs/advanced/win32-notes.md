...
The framework-side opt-in for legacy BitBlt exists: `WindowOptions.CaptureCompatible` (env override `GANTRY_CAPTURE_COMPAT=1/0`) renders without GPU compositing by passing `--disable-gpu-compositing`. `go-webview2` exposes no environment-options parameter (it calls `createCoreWebView2EnvironmentWithOptions` with `environmentOptions = 0` in `pkg/edge/chromium.go`), so `windowsEnvFixes` (`appshell/env_windows.go`) routes the flag through the `WEBVIEW2_ADDITIONAL_BROWSER_ARGUMENTS` environment variable, which the WebView2 loader honors - the same env-var pattern as the WSLg DMA-BUF fix on Linux, and it reaches widget/popup child processes for free because they inherit the environment. It costs GPU-accelerated rendering, so it stays opt-in.
