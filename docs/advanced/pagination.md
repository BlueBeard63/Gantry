# Dynamic routes & pagination

Most pages have a fixed path (`pages/settings` -> `/settings`). A dynamic page instead captures part of the URL, so one page serves many addresses - `/examples/page1/1`, `/examples/page1/2`, `/user/jack`, `/files/docs/intro`. This is how you build pagination, detail pages, and anything keyed by an id or slug, and it follows the same paired-folder convention as every other page.
...
Navigate to a concrete URL like any other route - `navigate("/examples/page1/2")` or `<Link to="/examples/page1/2">`. The same page component stays mounted as the id changes; `useParams()` re-renders it with the new value.
